...
object Helpers {
  ...
  def makeHeader(content: ReactElement, logout: () => Unit): ReactElement = {
  ...
  def basicLayout(content: ReactElement, logout: () => Unit, headerContent: ReactElement = div()): ReactElement = {
    Space()
      ...
      .size(sizePair(0, 48))(
        Layout()(
          Helpers.makeHeader(headerContent, logout),
          content,
